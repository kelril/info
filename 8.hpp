...
struct talk_to_svr
{
    ...
    void process_msg()
    { //Роль: Чтение ответа клиента и направление в соответствующую функцию
        std::string msg(buff_, already_read_);

        if (msg.find("login ") == 0)
            on_login(msg); // Регистрация
        else if (msg.find("ping ") == 0)
            on_ping(msg); // Пингумся
        else if (msg.find("clients ") == 0)
            on_clients(msg); // Список клиентов
        else
            std::cerr << "invalid msg " << msg << std::endl; // Некорректное сообщение
    }
    ...
    void on_ping(const std::string &msg)
    {
        ...
        std::string answer;
        in >> answer >> answer; //получение ответа от сервера
        // В случае получения client_list_changed, мы снова делаем запрос на получение листа клиентов.
    ...
    size_t read_complete(const boost::system::error_code &err, size_t bytes)
    {
        ...
        if (err)
            return 0;
        bool found = std::find(buff_, buff_ + bytes, '\n') < buff_ + bytes; //Убедились, что мы дочитали до симвода'\n'
        return found ? 0 : 1;
    }
...
void run_client(const std::string &client_name) // Подключение к серверу
{
    boost::asio::ip::tcp::endpoint ep(boost::asio::ip::address::from_string("127.0.0.1"), 8001); // Cоздаем адрес и порт к которому хотим подключиться
