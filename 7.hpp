...
public:
    ...
    void read_request() {   // Чтение запроса
        if (sock.available()) //если сокет доступен
            already_read_ += sock.read_some(boost::asio::buffer(buff_ + already_read_, MAX_MSG - already_read_)); //читается сообщение из сокета и заносится в buff_
        //already_read_-длина того, что мы считали из буфера(сообшения)
    ...
    void process_request() {    // Обработка полученого запроса
        ...
        // Парсим сообщение
        size_t pos = std::find(buff_, buff_ + already_read_, '\n') - buff_; //размер сообщения
        std::string msg(buff_, pos); //заносим в msg обрезанный буфер до /n
        std::copy(buff_ + already_read_, buff_ + MAX_MSG, buff_); //copy — это копирование диапазона ячеек и вставка его позицию buff_ //смещение
        already_read_ -= pos + 1;
    ...
    void on_login(const std::string &msg) {    // Регистрация пользователя
        std::istringstream in(msg); //превращение строки msg в поток
        in >> username_ >> username_; //записываем  в username значение потока in
    ...
    {       // Потокобезопасный доступ к вектору клиентов
            ...
            for (auto b = clients.begin(), e = clients.end(); b != e; ++b)
                msg += (*b)->username() + " "; //список подключенных клиентов
        }
        write("clients " + msg + "\n");
    }
    ...
    bool timed_out() const {    // Подсчет времени для отключения
        boost::posix_time::ptime now =
                boost::posix_time::microsec_clock::local_time(); //текущее время
...
void handle_clients_thread() {
    while (true) {
        ...
        // Потокобезопасный доступ к вектору клиентов
        boost::recursive_mutex::scoped_lock lk(mx); // mx - экземпляр мьютекс
